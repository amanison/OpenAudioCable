...
--*/
{
    ...
    // get pointer to virtual cable object from class factory method
    m_pVCable = CVCable::getCable(Capture_);
    ...
    // 
    // Initialize the loopback deice
    //
    ntStatus = m_pVCable->Initialize();
...
AKM: This function writes from the audio buffer to the associated loopback 
     buffer object which is shared by pairs (?) of streams. In the simple case,
     this is a 1-1 mapping. Later it might be enhanced to support 1-n, or even 
     n-n, but that would require more complex topology.
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        if ( m_pVCable ) m_pVCable->LoopData(m_pDmaBuffer + bufferOffset, runWrite);
...
AKM: This function reads from the loopback buffer object audio buffer and saves
     the data into the audio buffer. The loopback buffer is shared by pairs (?) 
     of streams. In the simple case, this is a 1-1 mapping. Later it might be 
     enhanced to support 1-n, or even n-n, but that would require more complex 
     topology.
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        if ( m_pVCable ) m_pVCable->WriteData(m_pDmaBuffer + bufferOffset, runWrite);
        bufferOffset = (bufferOffset + runWrite) % m_ulDmaBufferSize;
        ByteDisplacement -= runWrite;
    }
}
